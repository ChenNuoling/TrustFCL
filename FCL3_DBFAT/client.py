...
class Client:
    ...
    def dbfat_pgd(self, x, y, task_id):
        """
        DBFAT PGD: 计算每个样本到决策边界的最小误分类步数
        返回对抗样本和每个样本的最小迭代步数
        """
        training_mode = self.model.training
        self.model.eval()
        ...
        batch_size = x.size(0)
        min_steps = torch.full((batch_size,), self.args.dbfat_max_steps, device=self.args.device)
        x_adv = x.clone().detach() + torch.empty_like(x).uniform_(-self.args.pgd_eps, self.args.pgd_eps)
        x_adv = torch.clamp(x_adv, 0, 1).detach()

        for step in range(self.args.dbfat_max_steps):
            x_adv.requires_grad_(True)
            outputs = self.model(x_adv, task_id)
            pred = outputs.argmax(dim=1)
            
            misclassified = (pred != y) & (min_steps == self.args.dbfat_max_steps)
            min_steps[misclassified] = step + 1

            if (min_steps < self.args.dbfat_max_steps).all():
                break
    ...
    def dbfat_train_main_task(self, seq_idx, global_weights):
        """
        DBFAT 对抗训练：包含本地重加权和全局正则化
        1. 本地重加权：使用PGD最小误分类步数计算样本权重，距离边界越近权重越大
        2. 全局正则化：使用全局模型的logits作为正则化目标，引入KL散度损失
        """
        actual_task_id = self.task_sequence[seq_idx]
        ...
        for epoch in range(self.args.local_epochs):
            for x, y in loader:
                ...
                x_adv, min_steps = self.dbfat_pgd(x, y_map, seq_idx)

                d_all = min_steps.sum()
                weights = 1.0 - min_steps.float() / d_all

                optimizer.zero_grad()
                self.model.train()
                local_outputs = self.model(x_adv, seq_idx)

                with torch.no_grad():
                    global_outputs = global_model(x_adv, seq_idx)

                ce_loss = ce_criterion(local_outputs, y_map)
                weighted_ce_loss = (ce_loss * weights).mean()

                kl_loss = F.kl_div(F.log_softmax(local_outputs, dim=1),
                                   F.softmax(global_outputs, dim=1),
                                   reduction='batchmean')

                total_loss = weighted_ce_loss + self.args.dbfat_beta * kl_loss
                total_loss.backward()
                optimizer.step()
                ...
                with torch.no_grad():
                    total_correct += (local_outputs.argmax(dim=1) == y_map).sum().item()
                    total_samples += y.size(0)
