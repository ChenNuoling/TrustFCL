...
def main():
    ...
    params_str = f"clients{args.num_clients}_tround{args.t_round}_tasks{args.num_tasks}_alpha{args.dirichlet_alpha}_batch{args.batch_size}_lr{args.lr}_epochs{args.local_epochs}_dbfat_beta{args.dbfat_beta}_{now.strftime('%H%M%S')}"
    ...
    log_print("Initializing DBFAT FCL components with Randomized Task Sequences...")
    ...
    log_print("=== DBFAT Federated Continual Learning (Randomized Sequences) ===")
    ...
    for rnd in range(args.num_rounds):
        ...
        log_print("  > Step 1: DBFAT Local Adversarial Training")
        reports = []
        client_train_times = []
        client_mem_peaks = []
        client_mem_ends = []
        ...
        for client in clients:
            rep, loss, train_acc, train_time, mem_start, mem_peak, mem_end = client.dbfat_train_main_task(global_task_id, global_weights)
            reports.append(rep)
