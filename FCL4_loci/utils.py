...
class Args:
    def __init__(self):
        ...
        # KD (Knowledge Distillation) 设置
        self.kd_lambda = 1.0
        self.kd_epoch = 5

        # OT (Optimal Transport) 聚合设置
        self.use_ot_agg = False
        self.ot_similarity_k = 4
...
class Recorder:
    ...
    def output_task_switch(self, prev_task_id, curr_task_id):
        ...
        if self.args.record_comm_cost:
            ...
            self.log_print(f"  Task {prev_task_id} Communication Cost: {self.task_comm_rounds[prev_task_id]} rounds x {self.comm_cost_per_round/1024/1024:.2f} MB = {comm_cost/1024/1024:.2f} MB")
    ...
    def output_round_summary(self, rnd, all_results):
        ...
        if self.args.record_memory and len(self.memory_stats['client_train_peak']) > 0:
    ...
    def output_summary(self, round_acc_clean, round_acc_robust, global_task_id, total_time, output_dir):
        ...
        if self.args.record_comm_cost:
            ...
            for tid in sorted(self.task_comm_rounds.keys()):
                ...
                self.log_print(f"  Task {tid}: {rounds} rounds x {self.comm_cost_per_round / 1024 / 1024:.2f} MB = {cost / 1024 / 1024:.2f} MB")
        ...
        if self.args.record_memory and self.first_task_steady_mem is not None:
