...
Fcat方法总结：
1、在主干模型第l层后加入一个因果模型（小型全连接网络），通过广义矩估计（GMM）的统计方法，恢复出干净的特征
2、主干模型和因果模型训练，只上传因果模型的BN层参数
3、服务器端，根据样本数量和对抗损失大小加权平均聚合因果模型的BN层参数
"""
...
def main():
    ...
    log_print("Initializing FCL-Fcat components with Randomized Task Sequences...")
    task_gen = TaskGenerator(args)

    dummy_model = Init_model(args)
    server = Server(args)
    
    # 初始化全局因果模型BN参数（只聚合因果模型的BN层）
    global_causal_bn = None
    ...
    log_print("=== Federated Continual Learning Fcat (Randomized Sequences) ===")
    ...
    if global_causal_bn:
        comm_cost_per_round = get_model_param_size(global_causal_bn) * 2
    ...
    for rnd in range(args.num_rounds):
        ...
        log_print("  > Step 1: Fcat Local Training")
        reports = []
        losses = []
        client_train_times = []
        client_mem_peaks = []
        ...
        for client in clients:
            rep, loss, train_acc, adv_loss, train_time, mem_start, mem_peak, mem_end = client.fcat_main_train(global_task_id)
            reports.append(rep)
        ...
        global_causal_bn, agg_time, mem_start, mem_end = server.aggregate_bn(reports, losses)
        recorder.output_server_agg(agg_time)

        # 更新通信开销（首次聚合后计算）
        if comm_cost_per_round == 0 and global_causal_bn:
            comm_cost_per_round = get_model_param_size(global_causal_bn) * 2
        ...
        for client in clients:
            client.set_causal_bn(global_causal_bn)
