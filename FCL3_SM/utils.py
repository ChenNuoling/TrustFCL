...
class Args:
    def __init__(self):
        ...
        # CalFAT & Saliency Map (SM) 设置
        self.sm_lambda = 0.1  # SD-Loss权重
        self.ckl_lambda = 0.1  # CKL损失权重
        self.use_calfat_training = True  # 是否使用CalFAT训练
        self.k_sim_clients = 3  # SM_v3: 用于聚合的最相似客户端数量
...
def compute_class_prior(labels, num_classes):
    """
    计算类先验概率
    参数:
        labels: 标签张量, shape (N,)
    ...
    counts[unique_labels] = counts_labels.float()
    class_prior = counts / counts.sum()
    return class_prior


def compute_energy_normalized_saliency(grad_tensor):
    """
    对单个通道进行能量归一化
    参数:
        grad_tensor: 梯度张量, shape (C, H, W)
    返回:
        归一化后的 saliency map
    """
    eps = 1e-10
    # 计算每个通道的绝对值之和
    channel_sums = torch.sum(torch.abs(grad_tensor), dim=(1, 2), keepdim=True)
    # 归一化
    normalized = grad_tensor / (channel_sums + eps)
    return normalized
