...
"""
Slack方法总结：
1、引入一个 α-松弛参数 和选择前 K个小损失的客户端，对客户端权重进行调整：损失小的客户端权重放大（乘以 1+α）损失大的客户端：权重缩小（乘以 1−α）
2、（先不考虑这个场景）针对部分客户端使用标准训练、部分使用对抗训练的现实场景。采用分层聚合：先内层聚合标准训练的客户端，再与对抗训练的客户端进行 α-松弛聚合
"""
...
def main():
    ...
    log_print("Initializing Slack FCL components with Randomized Task Sequences...")
    ...
    log_print("=== Federated Continual Learning with Slack (Randomized Sequences) ===")
    ...
    for rnd in range(args.num_rounds):
        ...
        log_print("  > Step 1: Slack Local Training")
        reports = []
        losses = []
        client_train_times = []
        client_mem_peaks = []
        ...
        for client in clients:
            rep, loss, train_acc, train_time, mem_start, mem_peak, mem_end = client.slack_train_main_task(global_task_id, global_weights)
            reports.append(rep)
            losses.append(loss)
        ...
        log_print("  > Step 2: Global Aggregation (Slack)")
        global_weights, agg_time, mem_start, mem_end = server.aggregate(reports, losses)
