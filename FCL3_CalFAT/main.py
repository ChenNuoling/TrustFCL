...
CalFAT方法总结：
1、全部样本会转换为对抗样本，
2、训练时，使用CCE损失和CKL损失，其中，CCE损失时让模型关注少数类，
...
def main():
    ...
    log_print("Initializing CalFAT FCL components with Randomized Task Sequences...")
    ...
    log_print("=== Federated Continual Learning with CalFAT (Randomized Sequences) ===")
    ...
    for rnd in range(args.num_rounds):
        ...
        log_print("  > Step 1: CalFAT Local Training")
        reports = []
        client_train_times = []
        client_mem_peaks = []
        client_mem_ends = []
        ...
        for client in clients:
            rep, loss, train_acc, train_time, mem_start, mem_peak, mem_end = client.calfat_train_main_task(global_task_id, global_weights)
            reports.append(rep)
