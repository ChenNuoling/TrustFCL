...
FedALC总结：
1、对分类层输出的 logits 进行加权校准，使模型更关注少样本类别，采用的是平方根的倒数形，
即（类别样本数除以总样本数）再开平方根再倒数，将该权重乘到该类别对应的 logits 上
"""
...
def main():
    ...
    log_print("Initializing FedALC FCL components with Randomized Task Sequences...")
    ...
    log_print("=== Federated Continual Learning with FedALC (Randomized Sequences) ===")
    ...
    for rnd in range(args.num_rounds):
        ...
        log_print("  > Step 1: FedALC Local Training")
        reports = []
        client_train_times = []
        client_mem_peaks = []
        client_mem_ends = []
        ...
        for client in clients:
            rep, loss, train_acc, train_time, mem_start, mem_peak, mem_end = client.fedalc_main_train(global_task_id, global_weights)
            reports.append(rep)
