...
class Client:
    ...
    def fedalc_main_train(self, seq_idx, global_weights):
        """
        FedALC 训练：对抗训练 + logits 加权校准
        对分类层输出的 logits 进行加权校准，使模型更关注少样本类别
        权重计算：weight[c] = 1 / sqrt(count[c] / total_count)
        """
        actual_task_id = self.task_sequence[seq_idx]
        self.set_weights(global_weights)
        ...
        class_counts = torch.zeros(len(classes), device=self.args.device)
        for _, y in loader:
            for l in y:
                class_counts[label_map[l.item()]] += 1
        total_count = class_counts.sum()
        
        weights = torch.ones(len(classes), device=self.args.device)
        
        mask = class_counts > 0
        if mask.any():
            weights[mask] = 1.0 / torch.sqrt(class_counts[mask] / total_count + 1e-6)
        ...
        for epoch in range(self.args.local_epochs):
            for x, y in loader:
                ...
                loss = F.cross_entropy(weighted_outputs, y_map)
                loss.backward()
                optimizer.step()
