...
FAT方法实现：
1、本地对抗训练，每一个小批量（N个）中前K个为对抗样本，其余为干净样本；
2、每个任务训练时（每个任务通信20轮次），先用较小的K/N=0.1 训练前4次通信，
之后切换到K/N=0.8的训练剩余通信轮次。
"""
...
def main():
    ...
    log_print("Initializing FAT FCL components with Randomized Task Sequences...")
    ...
    log_print("=== Federated Continual Learning with FAT (Randomized Sequences) ===")
    ...
    for rnd in range(args.num_rounds):
        ...
        log_print("  > Step 1: FAT Local Training")
        reports = []
        client_train_times = []
        client_mem_peaks = []
        client_mem_ends = []

        # 计算当前任务内通信轮次和kn_ratio
        task_comm_round = rnd % args.t_round
        kn_ratio = 0.1 if task_comm_round < args.kn_ratio_change * args.t_round else 0.8
        log_print(f"    kn_ratio = {kn_ratio}")

        for client in clients:
            rep, loss, train_acc, train_time, mem_start, mem_peak, mem_end = client.fat_train_main_task(global_task_id, global_weights, kn_ratio)
            reports.append(rep)
