...
class Args:
    def __init__(self):
        ...
        # Sylva 超参数（针对SimpleCNN调整）
        self.sylva_beta = 0.5  # Shapley值中鲁棒性损失的权重（轻量级模型对扰动更敏感，保持0.5）
        self.sylva_B = 50  # Monte Carlo采样次数（4层只有24种排列，50次足够覆盖）
        self.sylva_p = 0.3  # 选择前p%的层进行二阶段训练（CNN只有4层，30%选择1-2层更合理）
        self.sylva_kl_weight = 0.1  # KL散度损失权重
        self.sylva_l2_weight = 1e-4  # L2正则化权重
        self.sylva_lr_ratio = 0.2  # 二阶段训练学习率比例（轻量级模型可适当增大）
        self.sylva_k_neighbors = 5  # 聚合时选择相似客户端的数量
...
class Recorder:
    ...
    def __init__(self, log_print, args, comm_cost_per_round=0):
        ...
        self.time_stats = {'client_train': [], 'server_agg': [], 'sylva_retrain': []}
        self.memory_stats = {
            'client_train_peak': [],
            'client_train_end': [],
            'server_agg_peak': [],
    ...
    def update_sylva_retrain_stats(self, retrain_time, retrain_mem_peak, retrain_mem_end):
        """更新Sylva二阶段训练统计"""
        self.time_stats['sylva_retrain'].append(retrain_time)
        self.memory_stats['sylva_retrain_peak'].append(retrain_mem_peak)
        self.memory_stats['sylva_retrain_end'].append(retrain_mem_end)
    ...
    def output_server_agg(self, agg_time):
        """输出服务器聚合信息"""
        self.log_print("  > Step 2: Global Aggregation (Sylva)")
    ...
    def output_round_summary(self, rnd, all_results, retrain_time=0, retrain_mem_peak=0, retrain_mem_end=0):
        """输出轮次总结"""
        ...
        if retrain_time > 0:
            self.log_print(f"              Sylva Retrain: {retrain_time:.2f}s")
        if self.args.record_memory:
            ...
            if retrain_mem_peak > 0:
                self.log_print(f"              Sylva Retrain Peak: {retrain_mem_peak:.2f}MB, Steady: {retrain_mem_end:.2f}MB")
    ...
    def output_summary(self, round_acc_clean, round_acc_robust, global_task_id, total_time, output_dir):
        ...
        if self.time_stats['sylva_retrain']:
            self.log_print(f"  Sylva Retrain Time: {np.sum(self.time_stats['sylva_retrain']):.2f}s")
        ...
        if self.args.record_memory:
            ...
            if self.memory_stats['sylva_retrain_peak']:
                self.log_print(f"  Sylva Retrain - Peak: {np.max(self.memory_stats['sylva_retrain_peak']):.2f}MB")
