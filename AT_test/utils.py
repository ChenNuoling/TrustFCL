...
class Args:
    def __init__(self):
        ...
        # 对抗训练参数
        self.at_epsilon = 8 / 255.0    # 对抗训练扰动幅度
        self.at_alpha = 2 / 255.0      # 对抗训练步长
        self.at_pgd_steps = 20         # 对抗训练PGD步数
        self.at_method = 'at'          # 对抗训练方法: at/trades/mart/gairat/fat/free_at/yopo/shared_at/uiat
        
        # TRADES 参数
        self.trades_beta = 6.0         # TRADES损失权重
        
        # MART (GCE) 参数
        self.mart_alpha = 1.0 / 3.0    # GCE引导指数α
        
        # GAIRAT 参数
        self.gairat_lambda = 0.0       # GAIRAT权重函数参数λ (∈[-1,0])
        self.gairat_burn_in = 30       # GAIRAT预热期轮数
        
        # FAT 参数
        self.fat_lambda = 0.1          # FAT特征对齐权重
        
        # Free-AT 参数
        self.free_at_steps_per_batch = 4  # Free-AT每batch步数
        
        # YOPO 参数
        self.yopo_m = 5                # YOPO全传播轮数
        self.yopo_n = 3                # YOPO每轮内扰动更新次数
        
        # Shared AT 参数
        self.shared_at_share_size = 8  # 堆大小s
        self.shared_at_kappa = -np.log(0.2)  # 损失截断
        self.shared_at_label_smoothing = 0.1  # 标签平滑
        
        # UIAT 参数
        self.uiat_lambda = 3.5         # UIAT权重λ
        self.uiat_epsilon_inv = 4 / 255.0  # 逆扰动半径ε'
        self.uiat_momentum = 0.9       # 动量γ
