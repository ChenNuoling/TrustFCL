...
'''
2026-06-04 17:30
根据数据集名字和args中的任务数、客户端数等参数，更换处理数据划分的方式
'''

class TaskGenerator:
    def __init__(self, args):
        self.args = args
        self.dataset_name = getattr(args, 'dataset', 'cifar100').lower()
    ...
    def _configure_tasks(self):
        """根据数据集类别数和任务数配置每个任务的类别范围"""
        self.classes_per_task = self.num_classes // self.num_tasks
        
        if self.num_classes % self.num_tasks != 0:
            print(f"Warning: {self.num_classes} classes cannot be evenly divided by {self.num_tasks} tasks")
            print(f"Using {self.classes_per_task} classes per task, {self.num_classes % self.num_tasks} classes will be ignored")
